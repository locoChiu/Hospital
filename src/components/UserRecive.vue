...
<template>
	<div class="reciveContainer">
		...
		<div class="reciveItem" v-for="item in list" :key="item.id">
			...
			<span class="name">{{item.UserCont}}&nbsp;购买</span>
			<span>&nbsp;{{item.PrjName}}&nbsp;</span>
			<span>&nbsp;消费：{{item.PrjBal}}元</span>
			<p class="reciveMoney">获得收益：<span>{{item.balCont}}元</span></p>
...
export default {
	...
	methods:{
		...
		getUserRecive(){	//获取客户收益数据函数
			var that = this;
			this.loginCheck(path.path_loginCheck,function(){
				that.$http.get(path.path_getUserRecive,{params:{id:that.id}}).then(res=>{
